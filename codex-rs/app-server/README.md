...
- `mcpToolCall` — `{id, server, tool, status, arguments, connectorId, mcpAppResourceUri?, mcpAppInvokedResourceUri, pluginId, result?, error?}` describing MCP calls; `connectorId` identifies the trusted MCP app connector, `mcpAppResourceUri` points to the widget template, and `mcpAppInvokedResourceUri` identifies the invoked MCP app resource. `status` is `inProgress`, `completed`, or `failed`.
