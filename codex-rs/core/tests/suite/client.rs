...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    for item in first_input {
        assert_eq!(item["metadata"]["turn_id"].as_str(), Some(first_turn_id));
    }
    ...
    let item_turn_id = |text: &str| {
        second_input
            ...
            .and_then(|item| item["metadata"]["turn_id"].as_str())
    };
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    ...
    let r3_input_array = requests[2].input_without_item_metadata();
