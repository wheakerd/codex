...
#[test]
fn with_user_config_profile_reorders_selected_profile_above_user_override() {
    let temp_dir = TempDir::new().expect("tempdir");
    let base_file = test_user_config_path(&temp_dir, "config.toml");
    let override_file = test_user_config_path(&temp_dir, "config.override.toml");
    let profile = "work".parse::<ProfileV2Name>().expect("profile name");
    let stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                ConfigLayerSource::User {
    ...
    let updated = stack.with_user_config_profile(
        &base_file,
        Some(&profile),
        toml::from_str(r#"model = "profile""#).expect("profile config"),
    );
