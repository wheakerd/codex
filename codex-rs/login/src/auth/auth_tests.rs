...
#[tokio::test]
async fn login_with_access_token_writes_agent_identity_record() {
    let dir = tempdir().unwrap();
    let auth_path = dir.path().join("auth.json");
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    mock_agent_task_registration(&server, "/backend-api", &record.agent_runtime_id, "task-id")
        .await;
    record.task_id = Some("task-id".to_string());
...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    assert_eq!(
        reloaded_agent_auth.run_task_id(),
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path(format!(
            "/backend-api/v1/agent/{}/task/register",
            record.agent_runtime_id
        )))
        .respond_with(ResponseTemplate::new(/*status*/ 503))
        .expect(/*requests*/ 3)
    ...
    assert!(
        !get_auth_file(dir.path()).exists(),
        "failed strict agent identity auth should not write auth.json"
    );
    Ok(())
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    mock_agent_task_registration(
        &server,
        "/backend-api",
        &expected_record.agent_runtime_id,
        "task-id",
    )
    .await;
    expected_record.task_id = Some("task-id".to_string());
    ...
    assert_eq!(agent_identity.run_task_id(), Some("task-id".to_string()));
    assert!(
